...
export function BookingForm({ roomId, roomNumber, price, paymentSettings }: BookingFormProps) {
  ...
  const { register, handleSubmit, watch, formState: { errors }, reset, trigger } = useForm<BookingFormValues>({
  ...
  const requiresPayment = paymentSettings?.enablePaymentOptions && (paymentSettings?.enableQrCode || paymentSettings?.enableStripe);
  ...
  return (
    ...
    >
      <div className="flex justify-between items-end border-b border-zinc-100 pb-4">
        <h3 className="font-nove text-2xl text-zinc-900">Reserve Room {roomNumber}</h3>
        <div className="flex flex-col items-end gap-1">
          <span className="text-xs uppercase tracking-widest text-zinc-400 font-medium leading-none">Rate</span>
          <span className="text-2xl font-medium text-zinc-900 leading-none">${price} <span className="text-sm text-zinc-500 font-light">/ Night</span></span>
        </div>
      </div>
      ...
      <AnimatePresence mode="wait">
        {step === 1 && (
          ...
          >
            ...
            <div className="bg-zinc-50 p-6 flex justify-between items-center border border-zinc-100 mt-4">
              <div>
                <span className="block text-xs uppercase tracking-widest text-zinc-500 font-medium">Total Estimate</span>
                <span className="text-sm text-zinc-400">{days} {days === 1 ? 'Night' : 'Nights'} at ${price}/night</span>
              </div>
              <div className="text-3xl text-zinc-900 font-medium">
                ${total.toFixed(2)}
              </div>
            </div>
        ...
        {step === 2 && (
          ...
          >
            ...
            <div className="bg-zinc-50 p-4 border border-zinc-100 flex justify-between items-center">
              <div>
                <span className="block text-xs uppercase tracking-widest text-zinc-500 font-medium">Required Fee ({paymentSettings?.reservationFeePercentage}%)</span>
              </div>
            ...
            {selectedPaymentMethod === 'QR' && paymentSettings?.enableQrCode && (
              <div className="flex flex-col gap-6 items-center text-center border border-zinc-100 p-6">
                ...
                <div className="w-full flex flex-col gap-2 text-left">
                  <label className="text-xs uppercase tracking-widest text-zinc-500 font-medium">Transfer Reference ID *</label>
                  <input 
                    type="text"
                    value={qrRefId}
            ...
            {selectedPaymentMethod === 'CREDIT_CARD' && paymentSettings?.enableStripe && paymentSettings.stripePublicKey && (
              <div className="mt-4">
        ...
        {step === 3 && (
          ...
          >
            ...
            <h3 className="font-nove text-2xl text-zinc-900">Booking Confirmed!</h3>
