  ...
  return (
    <section className="py-32 px-6 md:px-12 lg:px-24 bg-white">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <h2
            className="text-zinc-900 tracking-tighter uppercase mb-6"
            style={{ fontSize: "calc(var(--theme-heading-size) * 0.7)" }}
          >
            {settings.title}
          </h2>
          <p className="text-zinc-500 max-w-2xl mx-auto text-lg font-light">{settings.subtitle}</p>
        </motion.div>
          ...
          {settings.members.map((member, idx) => (
            ...
            >
              ...
              <div className="text-center">
                <h3 className="text-xl font-medium tracking-wide uppercase mb-2 text-zinc-900">{member.name}</h3>
                <p className="text-zinc-500 font-light uppercase tracking-widest text-sm">{member.role}</p>
              </div>
            </motion.div>
          ))}
        </div>
