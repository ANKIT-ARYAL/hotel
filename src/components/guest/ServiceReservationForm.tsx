...
import { createDiningReservation, createSpaReservation } from "@/app/actions/guest-reservations";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";

export function ServiceReservationForm({ type, options, trigger, open = false, initialOption, isLoggedIn = false }: { type: "spa" | "dining"; options: string[]; trigger?: ReactNode; open?: boolean; initialOption?: string; isLoggedIn?: boolean }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      else await createDiningReservation({ ...payload, restaurant: option });
  ...
  return <>
    ...
    {isOpen && <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 p-4" role="dialog" aria-modal="true">
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-card p-6 shadow-2xl md:p-10">
        <div className="mb-6 flex items-start justify-between gap-4"><div><h2 className="text-3xl font-semibold">Reserve {type === "spa" ? "a spa service" : "a dining table"}</h2><p className="mt-2 text-muted-foreground">Choose your preferred option and request a reservation.</p></div><button type="button" onClick={() => setIsOpen(false)} className="text-2xl text-muted-foreground" aria-label="Close">×</button></div>
        <form onSubmit={submit} className="grid gap-5">
          <label className="grid gap-2 font-medium">{type === "spa" ? "Treatment" : "Venue"}<select value={option} onChange={(e) => setOption(e.target.value)} className="h-12 rounded-md border bg-background px-3">{options.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
